...
function mockRouteFunctionFor(method) {
  ...
  return function(path, options, callback) {
    if ((options && options.constructor == Function) || (callback && callback.constructor == Function)) {
      return orig.apply(Express, arguments)
    }
    var request = mockRequest({ method : method.toUpperCase(), uri : { path : path }})
    if (options) {
      JSpec.extend(request, options.request)
      //restore the path, since the mockRequest defaults it to "/"
