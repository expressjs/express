...
(function(){
  Express = { 
    ...
    pathToRegexp : function(path) {
      ...
      Express.regexpKeys = []
      path = path.replace(/:(\w+)/, function(key){
    ...
    callRouteFor : function(request) {
      ...
      if (route.keys)
        for (var i = 0, len = route.keys.length; i < len; ++i) 
          this.request.uri.params[route.keys[i]] = Express.captures[i] || null
...
get ('/user/:name/edit', function(){
  return 'Editing user "' + (param('name') || 'none') + '"'
})
