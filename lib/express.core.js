...
Express = { 
  ...
  settings : {
    ...
    defaultRoute : {
      callback : function() {
        Express.halt('Page or file cannot be found', 'Not Found')
      }
    }
  },
  ...
  server : {
    ...
    finished: function(body) {
      ...
      Express.originalResponse.sendHeader(Express.response.status, Express.response.headers)
  ...
  redirect : function(uri) {
    ...
    this.halt("Page or file has moved to `" + uri + "'", 'Moved Temporarily')
  },
  ...
  /**
   * Set response _status_ and _body_. Status
   * defaults to 200.
   *
   * @param  {string} body
  ...
  halt : function(body, status) {
    ...
    Express.server.finished(this.response.body = body)
  },
  ...
  /**
   * Attempt to find and return a route matching _request_.
   *
   * @param  {object} request
   * @return {object}
   * @api private
