...
(function(){
  Express = { 
    ...
    callRouteFor : function(request) {
      ...
      // uri.params
      // headers [[foo, bar]]
      return (this.findRouteFor(request) || this.defaultRoute).callback(Express.passToRoute)
    },
      ...
      switch (route.path.constructor) {
        ...
        case RegExp: 
          if (captures = request.uri.path.match(route.path))
            return Express.passToRoute = captures
      }
    },
...
get (/foo(bar)?/, function(captures){
  Express.dump(captures)
  return 'test'
})
