...
exports.Request = new Class({
  ...
  /* Properties that should be blended/mixed in from the node.js request object */
  _blendedProperties: ['socket', 'httpVersion','headers',
                       'url', 'method',
                       'httpVersionMajor','httpVersionMinor',
                       'body','_encoding', '_events'],

  /** 
   * Given a node.js Request object, blend in the shared properties
   * for an express Request object
   * 
   * @param {Request} request The node.js Request object.
   * @api private
   */
  _blendInNodeRequest: function(request) {
     for(var i=0;i<this._blendedProperties.length;i++) {
       var prop=  this._blendedProperties[i]
