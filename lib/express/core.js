    ...
    request.trigger('request', function(err) {
      try {
        ...
        if (typeof (body = (new Router(request)).route()) === 'string')
          request.halt(200, body, null, function(err){
            if (err) Express.error(err, request, response)
          }) 
      } catch (err) {
        Express.error(err, request, response)
      }  
...
/**
 * Handle the given _err_ in relation to _request_ and _response_.
 *
 * @param {Error} err
 * @param {object} request
 * @param {object} response
...
Express.error = function (err, request, response) {
  if (!(request instanceof Request))
    request = new Request(request, response),
    request.trigger('request')
  if (request.accepts('html') && set('show exceptions'))
    request.halt(500, require('express/pages/show-exceptions').render(request, err))
  else
    request.halt(500)
