...
Router = Class({
  ...
  /**
   * Route #request returning the value returned by Route#run()
   *
   * @return {mixed}
   * @api private
   */
  ...
  route: function(){
    ...
    if (route)
      ...
      this.request.halt(404, require('express/pages/not-found').render())
    else
      this.request.halt()
  },
  ...
  route: function(request, response){
    ...
    try {
      ...
      if (request.accepts('html') && set('show exceptions'))
        request.halt(500, require('express/pages/show-exceptions').render(e))
      else
        request.halt(500)
