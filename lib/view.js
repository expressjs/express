...
View.prototype.lookup = function lookup(cb) {
  ...
  function tryNextRoot() {
    while (roots.length > 0) {
      ...
      // skip missing entries in a views array
      if (root == null) {
        continue;
      }
...
View.prototype.render = function render(options, callback) {
  ...
  try {
    ...
    // a synchronous throw happens before any stat is dispatched, so the
    // queue is still ours; reset it so future renders are not blocked
    this.pendingRenders = null;
    throw err;
  }
  ...
  function onLookup(filePath) {
    ...
    for (let i = 0; i < pending.length; i++) {
      if (filePath) {
        ...
        process.nextTick(pending[i][1], lookupFailedError(view));
      }
    }
  }
};
