...
View.prototype.lookup = function lookup(cb) {
  ...
  function tryNextRoot() {
    while (roots.length > 0) {
      ...
      // skip empty entries in a views array
      if (!root && root !== '') {
        continue;
      }
      ...
      // resolve the file
      return resolveView(dirname(loc), basename(loc), ext, onResolved);
    }
...
View.prototype.render = function render(options, callback) {
  ...
  try {
    ...
    // a synchronous throw must not leave the queue blocking future renders
    if (this.pendingRenders === queued) {
      this.pendingRenders = null;
    }
    throw err;
  }
  ...
  function onLookup(filePath) {
    ...
    for (let i = 0; i < pending.length; i++) {
      if (filePath) {
        ...
        pending[i][1](lookupFailedError(view));
      }
    }
  }
};
...
function statNext() {
  while (numPendingStats < MAX_PENDING_STATS && pendingStats.length > 0) {
    ...
    try {
      ...
      // release the slot and report it like an asynchronous stat error
      numPendingStats--;
      cb(err, null);
    }
  }
