...
app.render = function(view, options, fn){
  ...
  try {
    ...
    this.lookup(view, function(err, path){
      if (err) return fn(err);
      opts.filename = path;
      view = opts.cache
        ? cache[path] || (cache[path] = fs.readFileSync(path, 'utf8'))
        : fs.readFileSync(path, 'utf8');
      engine.render(view, opts, fn);
    });
  } catch (err) {
    fn(err);
  }
...
/**
 * Lookup the given `view` and callback `fn(err, path)`.
 *
 * @param {String} view
 * @param {String} fn
 * @api public
...
app.lookup = function(view, fn){
  var root = this.set('views') || process.cwd() + '/views';
  fn(null, join(root, view));
};