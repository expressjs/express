...
/**
 * Lookup `view` and invoke the callback `fn(err, path)`.
 *
 * @param  {View} view
 * @param  {Function} fn
 * @api private
...
exports.lookupView = function(view, fn){
  var path = view.path;

  // <path>.<engine>
  if (!isAbsolute(path)) path = join(view.root, path);
  ...
  exists(path, function(yes){
    ...
    exists(path, function(yes){
      if (yes) return fn(null, path);
      return fn(new Error('Failed to lookup view "' + view.name + '"'));
    });
  });
};