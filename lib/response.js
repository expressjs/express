 ...
 *     res.json({ user: {id: 'id01', pwd: 'pass'}, id: 'id01', name: 'abcd' }, ['id', 'name']);
 *     res.json(404, { code: 404, message: 'Not found', file: '/home/abcd/blah not found' }, ['code', 'message']);
 *     res.json({ code: 403, server: 'a=b failed', message: 'Invalid creds' }, ['code', 'message']);
 *
 * @param {Mixed} obj or status
 * @param {Mixed} obj or replacer
 * @param {Array} replacer
 * @return {ServerResponse}
 * @api public
 */

res.json = function(obj, replacer){
  // allow status / body
  if (2 <= arguments.length) {
  ...
  // settings
  var app = this.app;
  replacer = replacer || app.get('json replacer');
 ...
 *     res.json(404, { code: 404, message: 'Not found', file: '/home/gvaish/blah not found' }, ['code', 'message']);
 *     res.json({ code: 403, server: 'a=b failed', message: 'Invalid creds' }, ['code', 'message']);
 *
 * @param {Mixed} obj or status
 * @param {Mixed} obj or replacer
...
res.jsonp = function(obj, replacer){
  // allow status / body
  if (2 <= arguments.length) {
    // res.json(body, status) backwards compat
    if ('number' == typeof replacer) {
