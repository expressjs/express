...
var fs = require('fs')
  ...
  , normalize_type = require('./utils').normalize_type
...
res.format = function(obj){
  var key = req.accepts(Object.keys(obj))
    , req = this.req
    , next = req.next;

  if (key) {
    this.set('Content-Type', normalize_type(key));
    obj[key](req, this, next);
  } else {
    var err = new Error('Not Acceptable');
    err.status = 406;
    err.types = keys.map(normalize_type);
    next(err);
  }
