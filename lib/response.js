...
res.send = function send(body) {
  ...
  switch (typeof chunk) {
    ...
    case 'object':
      if (chunk === null) {
        ...
        if (!this.get('Content-Type')) {
          if(chunk.type) {
            this.setHeader('Content-Type', chunk.type);
          } else {
            this.type('bin');
          }
        }
  ...
  if (req.method === 'HEAD') {
    ...
    if (isChunkBlob) {
       ...
       */
      var blob = chunk;
      var destroyWriteStream = function (err) {
        if (!res.destroyed) {
          req.next(err);
        }
      }
      if(typeof blob.stream === 'function') {
        ...
        inputStream.on('error', destroyWriteStream);
        res.on('error',function(err){
          destroyWriteStream(err)
          inputStream.unpipe(res);
        });
        inputStream.pipe(res);
      } else {
        res.on('error', destroyWriteStream);
        blob.arrayBuffer()
        .then(function(arrayBuffer){
          res.end(Buffer.from(arrayBuffer));
        })
        .catch(destroyWriteStream)
      }
    } else {
