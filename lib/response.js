  ...
  if (typeof BigInt !== 'undefined') {
    try {
      ...
      if (err.message && err.message.includes('BigInt')) {
        var dominated = dominatedBigInt(value);
        if (dominated) {
          throw new TypeError('BigInt value at key "' + dominated + '" cannot be serialized by res.json(). ' +
            'Use a custom json replacer function or convert BigInt to string/number.');
        }
        throw new TypeError(err.message + '. Use a custom json replacer function or convert BigInt to string/number.');
      }
    }
  }
...
/**
 * Recursively find the first key path that contains a BigInt value.
 * Returns the key path string like "user.id" or "items[0].amount"
 *
 * @param {any} value
 * @param {string} [path]
 * @returns {string|null}
 * @private
 */
function dominatedBigInt (value, path) {
  if (value && typeof value === 'object') {
    if (Array.isArray(value)) {
      for (var i = 0; i < value.length; i++) {
        var result = dominatedBigInt(value[i], path ? path + '[' + i + ']' : '[' + i + ']');
      ...
      for (var j = 0; j < keys.length; j++) {
        ...
        if (v && typeof v === 'object') {
          var result = dominatedBigInt(v, newPath);
          if (result) return result;
        }
      }
    }
