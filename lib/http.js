...
Server.prototype.use = function(route, middleware){
  ...
  // Bind a handler for nested server middleware. This logic is lifted
  // out of connect's Server.prototype.use() and modified to add a
  // custom `out` callback.
  //
  // The `res.app` and `req.app` properties are set by the first entry
  // in the middleware stack (see Server() constructor). When a nested
  // server context is entered, the `.app` property is changed, but
  // it's not restored when `.handle()` calls `out()`.
  //
  // A different approach might be to add `begin`, `end`, `suspend`,
  // and `resume` events to servers.
