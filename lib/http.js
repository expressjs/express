...
app.local = function(name, val){
  if (1 == arguments.length) return this._locals[name];
...
/**
 * Register the given dynamic view locals `obj`. This method
 * can be called several times to apply additional locals.
 *
 * @param {Object} obj
 * @return {Server} for chaining
 * @api public
...
app.dynamicLocals = function(obj){
  for (var key in obj) {
    this.dynamicLocal(key, obj[key]);
  }
  return this;
};

/**
 * Register a dynamic local with `name` and callback `fn(req, res)`.
 *
 * @param {String} name
 * @param {Object} fn
 * @return {Server} for chaining
...
app.dynamicLocal = function(name, fn){
  this._dynamicLocals[name] = fn;
  return this;
};
